...
<c:set var="main" value="${fn:replace(url, pageContext.request.requestURI, '/scribbling')}"/>
   		...
   		<ul class="nav navbar-nav navbar-right">
   			...
   			<c:if test="${uno==null}">
   				...
   				<c:if test="${pageContext.request.requestURI != path}">
   					<li><a href='${pageContext.request.contextPath}/account/login_page' class="btn-nav-light">로그인</a></li>
   				</c:if>
   				<c:set var="path" value="${pageContext.request.contextPath}/user/register.jsp" />
   				<c:if test="${pageContext.request.requestURI != path}">
    				<li><a href='${pageContext.request.contextPath}/account/join_page' class="btn-nav">회원가입</a></li>
    			</c:if>
      		</c:if>
        	<c:if test="${uno!=null}">
